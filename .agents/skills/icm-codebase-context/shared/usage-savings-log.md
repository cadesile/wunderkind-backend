...
- **Tasks logged:** {{TASKS_LOGGED_COUNT}}
- **Cumulative estimated tokens saved:** {{CUMULATIVE_TOKENS_SAVED}}
...
## Recent tasks (most recent {{MAX_ENTRIES}} kept)

| Date | Stage(s) consulted | Task | Est. tokens saved |
|---|---|---|---|
| {{DATE}} | {{STAGES}} | {{TASK_SUMMARY}} | {{TOKENS_SAVED}} |
...
first, then append the row. If the table now has more than
{{MAX_ENTRIES}} rows, drop the oldest row(s) down to that cap — its value
is already folded into the running total above, so removing it loses no
data, only the per-task detail.
