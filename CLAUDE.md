...
- `hairColor`, `lip`, and `primary`/`secondary` (`KitColor`, one shared 12-color palette for both player kits and staff outfits — no more vibrant-player-vs-muted-staff split) are hex-backed enums, same pattern as `SkinId`'s predecessor.
- `face` is a fixed, generated-once expression (`App\Enum\Appearance\Face`, 9 cases) — a pure cosmetic pick, **not** tied to morale or personality. This is a deliberate simplification versus the old design, where eyes/mouth were computed live from morale at render time; the sprite has no such live coupling.
...
- `badgeCentre` allows `none` as a real, admin-settable choice, but the **generator and the widget's Randomise button both exclude it** — a blank badge centre isn't a useful thing to roll randomly. Randomise also rolls `badgeFill` as 50% chance equal to the **home** kit's primary (not away's).
