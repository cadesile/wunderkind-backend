...
-- Agents are NOT truncated here. Universal agents (isUniversal = true) are cleared
-- by `app:generate-market-data --clear` before regeneration. Non-universal (academy-
-- assigned) agents are preserved across resets since they carry no academy FK.
...
# Run app:generate-market-data FIRST (--clear removes universal agents/scouts/
# investors/sponsors before regenerating). Then run app:market:generate to add
# pool players, coaches, and more agents/scouts on top.
echo ""
echo -e "${BLUE}🌱 Phase 3a: Generating market data (agents · scouts · investors · sponsors)...${NC}"
lando php bin/console app:generate-market-data --clear
...
echo "   Cleared    : academies, players, guardians, staff, scouts, sponsors, investors,"
echo "                transfers, leaderboard entries, sync records, inbox messages, facilities"
echo "   Note       : Agents preserved (universal agents cleared/regenerated by seeder)"
echo "   Regenerated: market data  (25 agents · 30 scouts · 20 investors · 40 sponsors)"
echo "                market pool  (+30 agents · +100 pool players · +20 coaches · +10 scouts)"
echo "                totals       (55 agents · 40 scouts · 100 pool players · 20 coaches)"
echo ""
echo "   Backups    :"
